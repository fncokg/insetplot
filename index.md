...
insetplot is an R package to create ggplot2 maps with inset maps easily
and flexibly. It handles spatial configuration, aspect ratios, and plot
...
Use the same plot for the main map and all insets — let insetplot handle
...
- [`inset_spec()`](https://fncokg.github.io/insetplot/reference/inset_spec.md)
  ...
  - position: `loc` (e.g., “left bottom”) or `loc_left`/`loc_bottom` in
    \[0, 1\]
...
- [`ggsave_inset()`](https://fncokg.github.io/insetplot/reference/ggsave_inset.md)
  ...
  - Provide one of `width` or `height`; the other is computed from
    `main_ratio`
  - Optional `ratio_scale` for small adjustments (e.g., legends)
- [`map_border()`](https://fncokg.github.io/insetplot/reference/map_border.md)
  — Small theme to draw a rectangular border around plots
